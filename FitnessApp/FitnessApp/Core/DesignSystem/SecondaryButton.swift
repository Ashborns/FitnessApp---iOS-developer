// PULSE Design System — SecondaryButton
// Reusable secondary action button built entirely on Design_System tokens.
// Outlined with the `themeBorder` token over `themeSurface`; disabled state is
// visually distinct, disables hit testing, and never triggers its action.

import SwiftUI

/// A secondary action button rendered as an outlined surface.
///
...
struct SecondaryButton: View {
    ...
    /// Opacity applied to the enabled surface.
    ...
    /// Outlined surface: `themeSurface` fill with a `themeBorder` stroke, using
    /// the large corner-radius token. Kept in a `@ViewBuilder` helper so styling
    /// never relies on `AnyView`.
    @ViewBuilder
    private var background: some View {
        RoundedRectangle(cornerRadius: .cornerRadiusLarge, style: .continuous)
            ...
            .overlay(
                RoundedRectangle(cornerRadius: .cornerRadiusLarge, style: .continuous)
                    .stroke(Color.themeBorder, lineWidth: borderWidth)
            )
    }
    ...
    static var previews: some View {
        VStack(spacing: .spacingLarge) {
            SecondaryButton(
                ...
                identifier: "secondaryButton.add",
                action: {}
            )
            SecondaryButton(
                title: "Add to Routine",
                isEnabled: false,
                label: "Add to routine",
                identifier: "secondaryButton.addDisabled",
                action: {}
            )
        }
